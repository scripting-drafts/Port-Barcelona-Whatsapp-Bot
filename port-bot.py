    ...
    try:
        ...
        if user_id not in [*data]:
                ...
                try:
                    ...
                    if len(unread) > 0:
                        ...
                        try:
                            ...
                            bot_message = ''
                        if next_user_id != user_id and bot_message == '':
            ...
            while len(data[user_id]) == 2 and cnt:
                ...
                if message.text.lower() in vIncidents[vLangs.index(data[user_id][1])] and bot_message == '':
                    if message.text.lower() == vIncidents[vLangs.index(data[user_id][1])][7]:
                        ...
                        text_box.send_keys(picReq[vLangs.index(data[user_id][1])])
                        data[user_id].append(inc_type)
                        data[user_id].append(inc_type)
                        get_image[user_id] = True
                        user_queue.remove(user_queue[0])
                        break
                    else:
                ...
                try:
                    ...
                    if len(unread) > 0:
                        ...
                        if next_user_id != user_id and bot_message == '':
                            next_text_box = driver.find_element_by_css_selector("._2FVVk._2UL8j > ._3FRCZ.copyable-text.selectable-text")
                            if next_user_id in [*data]:
                                next_text_box.send_keys(wait[vLangs.index(data[next_user_id][1])])
                            else:
                                next_text_box.send_keys(wait[0] + wait[1] + wait[2] + wait[3])
                except Exception:
                    pass
            ...
            while len(data[user_id]) == 4 and cnt:
                if get_image[user_id]:
                    try:
                        image_context = driver.find_elements_by_css_selector('._2kLly > ._39rvu')[-1]
                        data[user_id][0] = str('{:>016d}'.format(x))
                        image_context.click()
                        sleep(3)
                        pic = driver.find_element_by_css_selector('._2kLly > ._8Yseo._3W6yC._3Whw5')
                        screenshot = pic.screenshot(data[user_id][0] + '.png')
                        ActionChains(driver).send_keys(Keys.ESCAPE).perform()
                        text_box = driver.find_element_by_css_selector("._2FVVk._2UL8j > ._3FRCZ.copyable-text.selectable-text")
                        text_box.send_keys(pro[vLangs.index(data[user_id][1])])
                        upload = Popen('drive push -quiet ' + data[user_id][0] + '.png', stdin=None, stderr=None, shell=True).communicate()
                        publish = Popen('drive pub ' + data[user_id][0] + '.png', stdin=None, stderr=None, stdout=PIPE, shell=True).communicate()
                        parts = str(publish).split('id=')
                        pub_id = str(parts[1]).split('\\n')
                        public_id = pub_id[0]
                        data[user_id].append('https://drive.google.com/file/d/' + public_id + '/preview')
                        text_box.send_keys(locMSG[vLangs.index(data[user_id][1])])
                        x += 1
                        get_image[user_id] = False
                        user_queue.remove(user_queue[0])
                        break
                    except Exception:
                        pass
            ...
            while len(data[user_id]) == 5 and cnt:
                try:
                    ...
                    response = requests.post('http://127.0.0.1:8000/input/' + user_id + '/', headers={ 'content-Type': 'application/json'}, data= '{"id":"' + data[user_id][0] + '", "inc_type":"' + data[user_id][2] + '", "inc_detail":"' + data[user_id][3] + '", "lat":"' + data[user_id][5] + '", "lon":"' + data[user_id][6] + '", "url":"' + data[user_id][7] + '", "pic":"' + data[user_id][4] + '", "is_active":"True"}')
                    if '400' in response or '422' in response:
                    ...
                    user_boxes = driver.find_elements_by_css_selector('._3ko75._5h6Y_._3Whw5')
                    for user_box in user_boxes:
                        if user_box.text.replace('+', '00').replace(' ', '') == user_id:
                            ActionChains(driver).context_click(user_box).perform()
...
# unread = '._31gEB'
# user_id = '.DP7CM > ._3ko75._5h6Y_._3Whw5'
# text_box = '._2FVVk._2UL8j > ._3FRCZ.copyable-text.selectable-text'
