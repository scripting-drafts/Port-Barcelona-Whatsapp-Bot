...
while True:
    ...
    try:
        ...
        if id not in [*data]:
            ...
            data[id] = [str(id) + str(ids[x]).zfill(7)]
            x += 1
            ...
            if len(data[id]) == 1:
                ...
                try:
                    ...
                    response = requests.post('http://127.0.0.1:8000/input/' + id + '/', headers={ 'content-Type': 'application/json'}, data= '{"id":"' + data[id][0] + '", "inc_type":"' + data[id][2] + '", "inc_detail":"' + data[id][3] + '", "lat":"' + data[id][4] + '", "lon":"' + data[id][5] + '"}')
