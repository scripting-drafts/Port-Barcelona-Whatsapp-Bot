...
@app.put("/update/{item_id}/", tags=["update"])
...
# curl -X PUT "http://127.0.0.1:8000/update/0000000000000000/"
...
def dashboard(skip: int = 0, db: Session = Depends(get_db)):
    ...
    <table style="width:100%">
        ...
        <td>{{ item['owner_id'] }}</td>
            {% if item['is_active'] %}
                <td><button class="button button1">Activo</button></td>
            {% else %}
                <td><button class="button button2">Resuelto</button></td>
            {% endif %}
        </tr>
        {% endfor %}
    </table>
    ...
    #template.globals['change'] = change
